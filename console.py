...
with st.container():
    ...
    with tab1:
        ...
        if st.button("Agro"):
            st.switch_page("pages/3_📊_DataFrame.py")
        if st.button("_Inspiration"):
            st.switch_page("pages/3_📊_DataFrame.py")
        if st.button("_Trivia"):
            st.switch_page("pages/3_📊_DataFrame.py")
...
c = st.container()
st.write("This will show last. Does this expand to the full extent of the container?  Let's seeeeeeeeeeeeeeeeeeee.")
st.bar_chart(np.random.randn(50, 3))
